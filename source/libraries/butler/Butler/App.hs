...
appCall :: Typeable a => Typeable b => AppInstance -> SyncName -> a -> ProcessIO (Maybe b)
appCall appInstance name message = do
    mvReply <- newEmptyTMVarIO
    writePipe appInstance.pipe (AppSync (SyncEvent name (toDyn message) mvReply))
